...
# Install minimal system dependencies (xvfb, ffmpeg, curl for healthcheck, and
# core Chromium shared libraries) (with cache)
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,target=/var/lib/apt,sharing=locked \
...
# This command will be inherited by the final stage. Auth and other settings
# are controlled entirely via environment variables — see docker-compose.yaml
# for the full list.
CMD ["sh", "-c", "Xvfb :99 -screen 0 1280x720x24 -nolisten tcp & sleep 1 && exec aniworld --web-ui --web-expose --no-browser --web-port ${ANIWORLD_WEB_PORT:-8080}"]
